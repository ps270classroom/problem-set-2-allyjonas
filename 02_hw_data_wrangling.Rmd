...
## There are 7014 candidates in the dataset
...
party_incumbent_promote <- fb_ads |> 
  ...
  summarize(promote_prop = mean(ad_tone_promote))

knitr::kable(party_incumbent_promote, col.names = c("party", "incumbency", "promote_prop"))
```
...
fb_ads <- fb_ads |>
  ...
  plot_q3 <- ggplot(data = fb_ads |> filter(office == "US House"), mapping = aes(impressions_millions)) + geom_histogram(na.rm = TRUE)
plot_q3
```
...
fb_top_plot <- ggplot(data = top_15_candidates,
  ...
  geom_col(aes(fill = party)) + labs(x = "Facebook Ad Impressions (in millions)",
                                     y = "Candidate Last Names (ascending from least to most impressions)",
